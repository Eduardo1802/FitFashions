...
const Equipamiento = () => {
  ...
  const recuperarDatosLocalmente = async () => {
    try {
      ...
      if (datos !== null) {
        ...
        const datosFiltrados = datosParseados.filter(item => item.categoria === 'nutricion');
        ...
        const productoData = productoSnapshot.docs.filter((doc) => doc.data().categoria === "nutricion");
  ...
  //     (doc) => doc.data().categoria === "nutricion"
  //   );
  //   setProyectos(data);
  //   // console.log(data)
  //   setDocs(data);
  ...
  const handleSearch = async () => {
    try {
      const datos = localStorage.getItem('productos');
      const datosParseados = JSON.parse(datos);
      let filteredData = datosParseados.filter(item => item.categoria === 'nutricion');
        ...
        <Grid item xs={12}>
          <Bread
            migas={[
              ...
              { miga: "NUTRICIÓN", ruta: "/tienda/nutricion", icono: <LocalCafeIcon /> },
            ]}
          />
        </Grid>
      </Grid>
