...
const Ropa = () => {
  ...
  const recuperarDatosLocalmente = async () => {
    try {
      ...
      if (datos !== null) {
        ...
        const datosFiltrados = datosParseados.filter(item => item.categoria === 'ropa');
        ...
        const productoData = productoSnapshot.docs.filter((doc) => doc.data().categoria === "ropa");
  ...
  const handleSearch = () => {
    try {
      const datos = localStorage.getItem('productos');
      const datosParseados = JSON.parse(datos);
      let filteredData = datosParseados.filter(item => item.categoria === 'ropa');
